...
//    swerve.postTrajectory(example);
  ...
  public static CommandBase otherexampleauto(SwerveSubsystem swerve)
  {
    boolean               onTheFly = false; // Use the path defined in code or loaded from PathPlanner.
    PathPlannerTrajectory example;
    if (onTheFly)
...
// position, heading(direction of travel), holonomic rotation
    ...
    {
      List<PathPlannerTrajectory> example1 = PathPlanner.loadPathGroup("other Path", new PathConstraints(4, 3));
      // This is just an example event map. It would be better to have a constant, global event map
      // in your code that will be used by all path following commands.
      HashMap<String, Command> eventMap = new HashMap<>();
      eventMap.put("marker1", new PrintCommand("Passed marker 1"));
